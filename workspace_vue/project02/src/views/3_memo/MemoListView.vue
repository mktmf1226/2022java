<template>
  <div class="container">
    ...
    <ul class="list-group">
      ...
      <li v-for="memo in memoList" v-bind:key="memo.memono" class="list-group-item">
        {{ memo.subject }}<span class="badge">{{ memo.readcnt }}</span>
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    getList() {
      ...
      // fetch('http://localhost:9000/memolist').then(alert())
